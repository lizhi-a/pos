...
function printReceipt(inputs){
    ...
    let string = '***<没钱赚商店>收据***' + '\n';
    string = string + '打印时间：' + n + '年' + y + '月' + r +'日'+ ' ' + h + ':' + m + ':' + s + '\n';
    string = string + '----------------------' + '\n';
    ...
    for(let i=0; i<list.length; i++)
    {
        ...
        {
            string = string + '名称：' + list[i].name + '，' + '数量：' + list[i].count + list[i].unit +'，' + '单价：' + parseFloat(list[i].price).toFixed(2) + '(元)' +'，' + '小计：' + parseFloat(list[i].total).toFixed(2) +  '(元)' + '\n';
            sum = sum + list[i].total;
        }
        
    }
    string = string + '----------------------' + '\n' + '总计：' + parseFloat(sum).toFixed(2) +  '(元)' + '\n' + '节省：' + parseFloat(save).toFixed(2) +  '(元)' + '\n' + '**********************';
    console.log(string);
}
